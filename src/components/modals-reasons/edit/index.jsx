...
const EditReasonModal = () => {
  ...
  const { setModalEdit, currentReason, setCurrentReason } = useModalReason();
  const tokenLoaded = Cookies.get("token");
  const [alert, setAlert] = useState(null);
  const [loading, setLoading] = useState(false);
  const [selectedType, setSelectedType] = useState(
    currentReason.isIncome ? arrayTypes[0] : arrayTypes[1]
  );
  ...
  const submitHandler = (event) => {
    ...
    updateReason(formData, currentReason.id, tokenLoaded)
      .then((res) => {
            ...
            <div className="flex flex-col gap-5">
              <div className="">
                ...
                <input
                  ...
                  defaultValue={currentReason.name}
